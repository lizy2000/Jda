...
public class SQLTestRunner {
	 ...
	 * 默认配置文件名
	 */
	private static String defaultFilename="SQLTestCase.properties";
	...
	private static Class[] getTestCaseClasses(String caseFile)throws Exception{
		...
		try {
			Properties properties = new SQLTestProperties();
			propertiesStream = SQLTestRunner.class.getResourceAsStream(caseFile);
	...
	public static void run(Class testClass)throws Throwable{
		if (testClass != null) {
			((SQLTestCase)testClass.newInstance()).run();
		}
	}
	...
	public static void main(String[] ags)throws Throwable{
		...
		SQLTestRunner.run(getTestCaseClasses());
