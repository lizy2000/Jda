 ...
 * @author Chris
 */
public class CalendarDateTimeHandler extends ObjectHandler {
  ...
  public void set(PreparedStatement ps,int index,Object value,int typeCode,JdaTypeConvertFactory convertFactory)throws SQLException{
   try{
  	Calendar parameter =(Calendar)convertFactory.convert(value,this.getPersisterType());
  	if(parameter== null)
  	  this.setNullParamValue(ps,index,typeCode);
  	else
  		ps.setTime(index,new java.sql.Time(parameter.getTime().getTime()));
   } catch (Throwable e) {
